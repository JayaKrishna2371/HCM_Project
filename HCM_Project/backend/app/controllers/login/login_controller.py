...
from app.auth.login.ldap_auth import LDAPAuthError, LDAPConfigError, authenticate
from app.auth.login.security import TokenError, create_access_token, decode_access_token
from app.db.session.session import get_db
from app.auth.login.deps import get_current_user
from app.models.tenants.tenant_model import TenantStatus
from app.models.users.user_model import User
from app.schemas.login.login_request import LoginRequest
from app.schemas.login.login_response import TokenIntrospectionResponse, TokenResponse
...
def login(body: LoginRequest, request: Request, db: Session = Depends(get_db)) -> TokenResponse:
    # Auto-detect the credential source: if a provisioned LOCAL account exists for
    # this username, authenticate it against the DB (bcrypt). Otherwise fall back
    # to Active Directory over LDAP — the platform's default for AD-backed users.
    #
    ...
    local_user = user_service.get_local_user(db, body.username)
    if local_user is not None:
        user = user_service.authenticate_local(db, local_user, body.password)
        if user is None:
            audit_service.record(
                db, action="auth.login", status="FAILURE", tenant_id=local_user.tenant_id,
                actor_user_id=local_user.id, actor_username=local_user.username,
                detail={"reason": "bad password", "auth_type": "local"}, request=request,
            )
            raise HTTPException(
        ...
        try:
            ...
            audit_service.record(
                ...
                actor_username=body.username, detail={"reason": str(e), "auth_type": "ldap"},
                request=request,
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    audit_service.record(
        ...
        detail={
            "role": primary_role,
            "super_admin": rbac.is_super_admin(user.roles),
